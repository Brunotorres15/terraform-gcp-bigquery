provider "google" {
  project     = "seu-projeto-no-gcp"
  region      = "us-west1"
}

resource "google_bigquery_dataset" "dsa_dataset" {
  dataset_id                  = "bt_dw_dataset"
...
resource "google_bigquery_job" "job_sql_1" {
  ...
  load {
    source_uris = [
      "gs://<seu-bucket-no-gcp>/tbClienteDSA.csv",
    ]
...
resource "google_bigquery_job" "job_sql_2" {
  ...
  load {
    source_uris = [
      "gs://<seu-bucket-no-gcp>/tbProdutoDSA.csv",
    ]
...
resource "google_bigquery_job" "job_sql_3" {
  ...
  load {
    source_uris = [
      "gs://<seu-bucket-no-gcp>/tbFatoDSA.csv",
    ]
